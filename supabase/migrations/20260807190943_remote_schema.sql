...
CREATE POLICY "owner can delete" ON public.characters
  FOR DELETE
  USING ((auth.uid() = user_id));

CREATE POLICY "owner can insert" ON public.characters
  FOR INSERT
  WITH CHECK ((auth.uid() = user_id));

CREATE POLICY "owner can read" ON public.characters
  FOR SELECT
  USING ((auth.uid() = user_id));

CREATE POLICY "owner can update" ON public.characters
  FOR UPDATE
  USING ((auth.uid() = user_id))
  WITH CHECK ((auth.uid() = user_id));
